...
import 'package:win_system_panel/features/power_control/application/power_controller.dart';
import 'package:win_system_panel/features/power_control/domain/power_task.dart';
import 'package:win_system_panel/i18n/l10n/app_localizations.dart';
...
class _PowerControlPageState extends ConsumerState<PowerControlPage> {
  ...
  //默认关机操作
  PowerOperation _selectedOperation = PowerOperation.shutdown;
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  isTaskRuning
                      ...
                      : _buildScheduleButton(t, colorScheme, controller),
                ],
              ),
            ),
          ),
  ...
  Widget _buildOperationSelector(
      AppLocalizations t, ColorScheme colorScheme, bool isTaskRunning) {
    // 创建操作类型列表，并映射到多语言文本
  ...
  Widget _buildScheduleButton(
      AppLocalizations t, ColorScheme colorScheme, PowerController controller) {
    return FilledButton.icon(
      onPressed: () {
        final delay = Duration(minutes: _selectedDelayMinutes.round());
        controller.schedule(_selectedOperation, delay);
      },
      label: Text(
